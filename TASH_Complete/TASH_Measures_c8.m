function [lh,rh]=TASH_Measures_c8(Dir,lh,rh,line)
...
for isub = 1:length(subjectsID)
    ...
    fname = fullfile(D_load,['lh_masked_thr01_Complete_expandedLT_Complete_erosion_dilatation_2_complete_g8_label']);
    fid = fopen(fname);
    for i=1:line
    fgetl(fid);
    end
    resp= textscan(ans,'lh_masked_thr01_Complete_expandedLT_Complete_erosion_dilatation_2_complete_g8.label %f %f %f %f %f %f %f %f %f');
    ...
    try
    D_load = fullfile(Dir,subjectsID{isub},'Volume');
    fname = fullfile(D_load,['rh_masked_thr01_Complete_expandedLT_Complete_erosion_dilatation_2_complete_g8_label']);
    fid = fopen(fname);
    for i=1:line
    fgetl(fid);
    end
    resp= textscan(ans,'rh_masked_thr01_Complete_expandedLT_Complete_erosion_dilatation_2_complete_g8.label %f %f %f %f %f %f %f %f %f');
