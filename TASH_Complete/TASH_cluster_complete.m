function TASH_cluster_complete(side,D,subjectsID)
...
for isub = 1:length(subjectsID)
   ...
   try
     ...
     it=0;
     for this_cluster=1:ic        
        if size(sorted_cluster{this_cluster},1)>=120
           ict = [ict this_cluster];
           it = it +1;
        end
     end
     
             
     curv_g=zeros(size(curv,1),it);
     for this_cluster=1:it
        curv_g(sorted_cluster{ict(this_cluster)},this_cluster)=curv(sorted_cluster{ict(this_cluster)});
     end
  ...
  for this_cluster=1:it
     ...
     fname = fullfile(D_load,[side, '_masked_thr0', th_string(3:end), suff1, '_expanded', expanded, suff2, '_erosion_dilatation_', erosion_dilatation_order, '_complete_g', g_num, '.curv']);
     disp(['saving ', fname]);
     ...
     fname = fullfile(D_load,[side, '_masked_thr0', th_string(3:end), suff1, '_expanded', expanded, suff2, '_erosion_dilatation_', erosion_dilatation_order, '_complete_g', g_num, '.w']);
     disp(['saving ', fname]);
     ...
     fname = fullfile(D_load,[side, '_masked_thr0', th_string(3:end), suff1, '_expanded', expanded, suff2, '_erosion_dilatation_', erosion_dilatation_order, '_complete_g', g_num, '.label']);
     fid = fopen(fname,'w');
     fprintf(fid, '# \n %d \n', NumTotalVertex);
     teste=0;
     ...
     fname = fullfile(D_load,[side, '_masked_thr0', th_string(3:end), suff1, '_expanded', expanded, suff2, '_erosion_dilatation_', erosion_dilatation_order, '_complete_g_all.curv']);
     disp(['saving ', fname]);
     write_curv(fname, curv,fnum);
     ...
     fname = fullfile(D_load,[side, '_masked_thr0', th_string(3:end), suff1, '_expanded', expanded, suff2, '_erosion_dilatation_', erosion_dilatation_order, '_complete_g_all.w']);
     disp(['saving ', fname]);
     write_wfile(fname, Curv_w, vcurv_index); 
     ...
     NumTotalVertex=length(vcurv_index);
     fname = fullfile(D_load,[side, '_masked_thr0', th_string(3:end), suff1, '_expanded', expanded, suff2, '_erosion_dilatation_', erosion_dilatation_order, '_complete_g_all.label']);
     fid = fopen(fname,'w');
     fprintf(fid, '# \n %d \n', NumTotalVertex);
     teste=0;
     for i=1:NumTotalVertex
